...
export class IndexingService {
	...
	constructor(private config: IndexingConfig, private eventBus: EventBus) {}
...
export class ContextService {
	...
	private async identifyPatternUsage(
		files: RelevantFile[],
...
export class BriefingService {
	...
	private async generateArchitectureGuidance(
		context: ContextPackage,
	): Promise<ArchitectureGuidance>;
	private async generatePatternGuidance(
		context: ContextPackage,
	): Promise<PatternGuidance[]>;
	private async generateImplementationGuidance(
		task: TaskDefinition,
		context: ContextPackage,
	): Promise<ImplementationGuidance>;
	private async generateTestingGuidance(
		task: TaskDefinition,
	): Promise<TestingGuidance>;
}
...
export const App: React.FC<AppProps> = ({
	...
	const [currentView, setCurrentView] = useState<'index' | 'context' | 'brief'>(
		'index',
	);
	const [projectIndex, setProjectIndex] = useState<ProjectIndex | null>(null);
	const [currentContext, setCurrentContext] = useState<ContextPackage | null>(
		null,
	);
	const [currentBrief, setCurrentBrief] = useState<ImplementationBrief | null>(
		null,
	);
...
describe('IndexingService', () => {
	...
	describe('indexProject', () => {
		it('should index all files in project', async () => {
			const result = await indexingService.indexProject(
