/**
 * Pattern recognition engine for GuardianAI MVP
 * 
...
export interface NamingConvention {
  type: 'camelCase' | 'PascalCase' | 'snake_case' | 'kebab-case' | 'CONSTANT_CASE'
  confidence: number
  examples: string[]
...
export class PatternMatcher {
   ...
   */
  private async detectNamingConventions(files: Map<string, FileInfo>): Promise<void> {
    ...
    for (const [filePath, fileInfo] of files) {
      ...
      variableNames.push(...fileInfo.exports
   ...
   */
  private async detectStructuralPatterns(files: Map<string, FileInfo>): Promise<void> {
    // Service pattern
  ...
  private detectServicePattern(files: Map<string, FileInfo>): void {
    ...
    if (serviceFiles.length > 0) {
      const pattern: StructuralPattern = {
        ...
        confidence: Math.min(0.95, serviceFiles.length / Math.max(1, files.size * 0.3))
      }
  ...
  private detectUtilityPattern(files: Map<string, FileInfo>): void {
    ...
    for (const [filePath, fileInfo] of files) {
      const isUtilFile = filePath.toLowerCase().includes('util') ||
    ...
    if (utilFiles.length > 0) {
      const pattern: StructuralPattern = {
        ...
        indicators: [...new Set(indicators)],
        confidence: Math.min(0.9, utilFiles.length / Math.max(1, files.size * 0.2))
      }
  ...
  private detectTestPattern(files: Map<string, FileInfo>): void {
    ...
    if (testFiles.length > 0) {
      const pattern: StructuralPattern = {
        ...
        files: testFiles,
        indicators: [...new Set(indicators)],
        confidence: Math.min(0.95, testFiles.length / Math.max(1, files.size * 0.4))
      }
  ...
  private detectConfigPattern(files: Map<string, FileInfo>): void {
    ...
    for (const [filePath] of files) {
      if (filePath.toLowerCase().includes('config') ||
    ...
    if (configFiles.length > 0) {
      const pattern: StructuralPattern = {
        ...
        files: configFiles,
        indicators: [...new Set(indicators)],
        confidence: Math.min(0.85, configFiles.length / Math.max(1, files.size * 0.1))
      }
  ...
  private detectTypePattern(files: Map<string, FileInfo>): void {
    ...
    if (typeFiles.length > 0) {
      const pattern: StructuralPattern = {
        ...
        indicators: [...new Set(indicators)],
        confidence: Math.min(0.9, typeFiles.length / Math.max(1, files.size * 0.15))
      }
   ...
   */
  private async detectCodePatterns(files: Map<string, FileInfo>): Promise<void> {
    for (const [filePath, fileInfo] of files) {
      try {
        ...
        const hasErrorHandling = fileInfo.functions.some(f => 
   ...
   */
  private async detectArchitecturalPatterns(files: Map<string, FileInfo>): Promise<void> {
  ...
  private detectMVCPattern(files: Map<string, FileInfo>): void {
    ...
    if (foundComponents.length >= 2) {
      const pattern: StructuralPattern = {
        ...
        confidence: Math.min(0.85, foundComponents.length / mvcComponents.length)
      }
  ...
  private detectModulePattern(files: Map<string, FileInfo>): void {
    ...
    const significantModules = Array.from(modules.entries())
    ...
    if (significantModules.length > 0) {
      const pattern: StructuralPattern = {
        ...
        indicators: significantModules.map(([name, files]) => 
          `${name} module (${files.length} files)`
        ),
        confidence: Math.min(0.8, significantModules.length / Math.max(1, modules.size * 0.5))
      }
   ...
   */
  private async consolidatePatterns(options: PatternDetectionOptions): Promise<Pattern[]> {
    ...
    for (const [key, convention] of this.namingConventions) {
      patterns.push({
        ...
        category: 'naming',
        examples: options.includeExamples ? convention.examples.map(example => ({
    ...
    for (const [key, structural] of this.structuralPatterns) {
      patterns.push({
        ...
        category: 'structural',
        examples: options.includeExamples ? structural.files.slice(0, 3).map(file => ({
    ...
    for (const [key, codePattern] of this.codePatterns) {
      patterns.push({
        ...
        category: 'code',
        examples: options.includeExamples ? codePattern.examples.map(example => ({
   ...
   */
  async matchPatterns(fileInfo: FileInfo, patterns: Pattern[]): Promise<PatternMatch[]> {
  ...
  private matchesStructuralPattern(fileInfo: FileInfo, pattern: any): boolean {
    if (typeof pattern === 'string') {
      return fileInfo.path.toLowerCase().includes(pattern.toLowerCase()) ||
             fileInfo.classes.some(c => c.name.toLowerCase().includes(pattern.toLowerCase()))
    }
    return false
  }
  ...
  private matchesUsagePattern(fileInfo: FileInfo, pattern: any): boolean {
    if (pattern instanceof RegExp) {
      ...
      return fileInfo.functions.some(f => 
