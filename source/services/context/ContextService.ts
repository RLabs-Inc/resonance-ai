...
import { GuardianError, ContextError } from '../../core/errors.js';
import { EventBus } from '../../core/events.js';
import { PatternMatcher } from './PatternMatcher.js';
import { RelevanceCalculator, RelevanceFactors } from './RelevanceCalculator.js';
import { extractKeywords } from '../../utils/text-utils.js';
...
export class ContextService {
  ...
  constructor(
    eventBus: EventBus,
    options: { customWeights?: Partial<import('./RelevanceCalculator.js').RelevanceWeights> } = {}
  ) {
  ...
  async compileContext(
    ...
    try {
      ...
      const relevanceFactors = this.buildRelevanceFactors(analyzedRequest, projectIndex);
      ...
      const detectedPatterns = options.includePatterns !== false
    ...
    for (const filePath of currentFiles) {
      ...
      for (const [path, otherFile] of Object.entries(projectIndex.files)) {
        if (otherFile.imports?.some(imp => 
          this.resolveImportPath(imp, projectIndex) === filePath
        )) {
  ...
  private inferTaskType(description: string): string {
    ...
    if (lowerDesc.includes('add') || lowerDesc.includes('create') || lowerDesc.includes('implement')) {
      return 'feature';
    }
    if (lowerDesc.includes('fix') || lowerDesc.includes('bug') || lowerDesc.includes('error')) {
      return 'bugfix';
    }
    if (lowerDesc.includes('refactor') || lowerDesc.includes('improve') || lowerDesc.includes('optimize')) {
      return 'refactor';
    }
  ...
  private buildRelevanceFactors(
    ...
    if (request.scope) {
      keywords.push(...extractKeywords(request.scope));
    }

    // Add constraint-based keywords
    if (request.constraints) {
      for (const constraint of request.constraints) {
        keywords.push(...extractKeywords(constraint));
      }
    }
  ...
  private buildDependencyInfo(
    ...
    for (const relevantFile of relevantFiles) {
      ...
      for (const [path, otherFile] of Object.entries(projectIndex.files)) {
        if (otherFile.imports?.some(imp => 
          this.resolveImportPath(imp, projectIndex) === file.path
        )) {
  ...
  private generateSummary(
    ...
    return `Context for ${taskType}: ${request.description}. ` +
           `Found ${fileCount} relevant files with average relevance ${
             fileCount > 0 
               ? (relevantFiles.reduce((sum, f) => sum + f.score, 0) / fileCount).toFixed(2)
               : '0.00'
           }.`;
  }
  ...
  private generateImplementationGuidance(
    ...
    switch (request.taskType) {
      case 'feature':
        guidance.push('1. Follow existing service patterns for new functionality');
      ...
      case 'refactor':
        ...
        break;
      default:
        guidance.push('1. Review the relevant files to understand current implementation');
    ...
    if (servicePattern && servicePattern.confidence > 0.7) {
      guidance.push('4. Follow the established service pattern with dependency injection');
    }

    const errorPattern = patterns.find(p => p.name === 'error-handling');
    if (errorPattern && errorPattern.confidence > 0.7) {
      guidance.push('5. Use the established error handling patterns with proper error context');
    }
  ...
  private calculateOverallConfidence(relevantFiles: RelevanceScore[]): number {
    ...
    const avgConfidence = relevantFiles.reduce((sum, f) => sum + f.confidence, 0) / relevantFiles.length;
    const scoreConfidence = relevantFiles.reduce((sum, f) => sum + f.score, 0) / relevantFiles.length;
    
    return (avgConfidence + scoreConfidence) / 2;
  }

  private resolveImportPath(importPath: string, projectIndex: ProjectIndex): string | null {
