...
import { calculateSimilarity, extractKeywords } from '../../utils/text-utils.js';
...
export class RelevanceCalculator {
   ...
   */
  calculateRelevance(file: FileInfo, factors: RelevanceFactors): RelevanceScore {
    try {
      const scores = {
   ...
   */
  calculateBatchRelevance(files: FileInfo[], factors: RelevanceFactors): RelevanceScore[] {
    return files.map(file => this.calculateRelevance(file, factors))
  ...
  filterByRelevance(
    ...
    return this.calculateBatchRelevance(files, factors)
  ...
  private calculateKeywordScore(file: FileInfo, keywords: string[]): number {
    ...
    const fileKeywords = [
      ...extractKeywords(file.name),
      ...extractKeywords(file.content || ''),
      ...(file.exports || []),
      ...(file.imports || [])
    ].map(k => k.toLowerCase());
    ...
    for (const keyword of normalizedKeywords) {
      ...
      for (const fileKeyword of fileKeywords) {
        const similarity = calculateSimilarity(keyword, fileKeyword);
  ...
  private calculatePatternScore(file: FileInfo, patterns: ContextPattern[]): number {
    ...
    for (const pattern of patterns) {
      const hasMatch = filePatterns.some(fp => 
        fp.type === pattern.type && 
        fp.name === pattern.name
      );
  ...
  private calculateFileTypeScore(file: FileInfo, targetTypes: string[]): number {
    if (targetTypes.length === 0) return 0.5; // Neutral if no preference

    const fileExtension = file.name.split('.').pop()?.toLowerCase() || '';
    return targetTypes.map(t => t.toLowerCase()).includes(fileExtension) ? 1 : 0;
  }

  private calculateDependencyScore(file: FileInfo, targetDependencies: string[]): number {
    ...
    for (const target of normalizedTargets) {
      const hasMatch = fileDependencies.some(dep => 
        dep.includes(target) || target.includes(dep)
      );
      if (hasMatch) matchCount++;
    }
  ...
  private calculateRecencyScore(file: FileInfo, targetDate: Date): number {
    ...
    const daysSince = (targetDate.getTime() - file.lastModified.getTime()) / (1000 * 60 * 60 * 24);
