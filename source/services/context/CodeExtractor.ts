...
export class CodeExtractor {
  ...
  extractSections(
    ...
    try {
      ...
      if (criteria.functions) {
        sections.push(...this.extractFunctions(sourceFile, content, criteria.functions, opts));
      }

      if (criteria.classes) {
        sections.push(...this.extractClasses(sourceFile, content, criteria.classes, opts));
      }

      if (criteria.interfaces) {
        sections.push(...this.extractInterfaces(sourceFile, content, criteria.interfaces, opts));
      }

      if (criteria.exports) {
        sections.push(...this.extractExports(sourceFile, content, criteria.exports, opts));
      }

      if (criteria.keywords) {
        sections.push(...this.extractByKeywords(sourceFile, content, criteria.keywords, opts));
      }

      if (criteria.lineRanges) {
        sections.push(...this.extractByLineRanges(content, criteria.lineRanges, opts));
      }
  ...
  private createCodeSection(
    ...
    const startLine = Math.max(0, start.line - (options.includeComments ? options.contextLines : 0));
  ...
  private calculateKeywordRelevance(line: string, keywords: string[]): number {
    ...
    for (const keyword of keywords) {
      ...
      const occurrences = (lowerLine.match(new RegExp(lowerKeyword, 'g')) || []).length;
      matches += occurrences;
    }
