/**
 * Main indexing service for GuardianAI MVP
 * 
...
export class IndexingService {
  ...
  async indexProject(
    ...
    try {
      ...
      const crawlResult = await this.crawler.crawl(rootPath, {
        includePatterns: [], // Use config defaults
        excludePatterns: [],
      ...
      const dependencyGraph = await this.dependencyBuilder.buildGraph(enrichedFiles)

      // Phase 4: Detect patterns
      await this.emitProgress('patterns', 0, 0)
      const patterns = options.skipPatterns ? [] : await this.detectPatterns(enrichedFiles)
  ...
  private async parseFiles(
    ...
    for (const [filePath, fileInfo] of targetFiles) {
      try {
        ...
        processed++
        await this.emitProgress('parsing', processed, targetFiles.length, filePath)
   ...
   */
  private async detectPatterns(files: Map<string, FileInfo>): Promise<Pattern[]> {
  ...
  private detectServicePattern(files: Map<string, FileInfo>): Pattern | null {
    const serviceFiles = Array.from(files.values()).filter(file => 
    ...
    return {
      ...
      examples: serviceFiles.slice(0, 3).map(file => ({
        ...
        code: file.classes.find(cls => cls.name.endsWith('Service'))?.name || 'Service',
        file: file.path,
  ...
  private detectUtilPattern(files: Map<string, FileInfo>): Pattern | null {
    const utilFiles = Array.from(files.values()).filter(file => 
    ...
    return {
      ...
      examples: utilFiles.slice(0, 3).map(file => ({
        ...
        code: file.functions.slice(0, 2).map(fn => fn.name).join(', '),
        file: file.path,
        lines: [1, 10] as [number, number],
  ...
  private detectTestPattern(files: Map<string, FileInfo>): Pattern | null {
    const testFiles = Array.from(files.values()).filter(file => 
  ...
  async updateIndex(changedFiles: string[]): Promise<void> {
    ...
    try {
      ...
      this.index.dependencies = await this.dependencyBuilder.buildGraph(this.index.files)
  ...
  private async updateSingleFile(filePath: string): Promise<void> {
    ...
    try {
      ...
      let fileInfo: FileInfo = {
        path: filePath,
        relativePath: FileUtils.getRelativePath(this.index.metadata.rootPath, filePath),
  ...
  private queryFiles(files: FileInfo[], filter?: QueryFilter): FileInfo[] {
    ...
    return files.filter(file => {
      ...
      if (filter.modifiedAfter && file.lastModified < filter.modifiedAfter) return false
  ...
  private queryPatterns(files: FileInfo[], filter?: QueryFilter): FileInfo[] {
    ...
    return files.filter(file => 
      file.patterns.some(pattern => pattern.patternId.includes(filter.hasPattern!))
    )
  }
  ...
  async saveIndex(
    filePath: string, 
    options: IndexPersistenceOptions = { format: 'json', compress: false, includeContent: false }
  ): Promise<void> {
    if (!this.index) {
  ...
  async loadIndex(filePath: string): Promise<ProjectIndex> {
    try {
      const content = await FileUtils.readFile(filePath)
      const indexData = filePath.endsWith('.yaml') || filePath.endsWith('.yml')
  ...
  getProjectStats(): any {
    ...
    for (const file of this.index.files.values()) {
      ...
      const complexity = file.complexity.cyclomatic
      const range = complexity < 5 ? 'low' : complexity < 10 ? 'medium' : 'high'
      complexityDistribution.set(range, (complexityDistribution.get(range) || 0) + 1)
    }
