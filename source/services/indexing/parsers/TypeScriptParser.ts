/**
 * TypeScript parser for GuardianAI MVP
 * 
...
export class TypeScriptParser implements LanguageParser {
      ...
      if (node.importClause.namedBindings) {
        if (ts.isNamespaceImport(node.importClause.namedBindings)) {
          ...
          const namedImports = node.importClause.namedBindings.elements.map(element => {
            return element.propertyName ? element.propertyName.text : element.name.text
          })
          imported.push(...namedImports)
          importType = imported.length === 1 && !node.importClause.name ? 'named' : 'named'
        }
      }
    }
    ...
    const visit = (node: ts.Node) => {
      ...
      if (ts.isVariableStatement(node) && this.hasExportModifier(node)) {
        for (const declaration of node.declarationList.declarations) {
          const exportInfo = this.createExportInfo(declaration, 'variable', sourceFile)
          if (exportInfo) exports.push(exportInfo)
        }
      }
   ...
   */
  private parseClassDeclaration(node: ts.ClassDeclaration, sourceFile: ts.SourceFile): ClassInfo | null {
    ...
    const implementsTypes = implementsClause?.types.map(type => type.getText(sourceFile)) ?? []
   ...
   */
  private parsePropertyDeclaration(node: ts.PropertyDeclaration, sourceFile: ts.SourceFile): PropertyInfo | null {
    const name = this.getNodeName(node)
    if (!name) return null
   ...
   */
  private parseInterfaceDeclaration(node: ts.InterfaceDeclaration, sourceFile: ts.SourceFile): InterfaceInfo {
    const name = node.name.text
    const line = sourceFile.getLineAndCharacterOfPosition(node.getStart()).line + 1
    const endLine = sourceFile.getLineAndCharacterOfPosition(node.getEnd()).line + 1
    
    const extendsTypes = node.heritageClauses?.find(
      clause => clause.token === ts.SyntaxKind.ExtendsKeyword
   ...
   */
  private parseMethodSignature(node: ts.MethodSignature, sourceFile: ts.SourceFile): MethodSignature | null {
    const name = this.getNodeName(node)
    if (!name) return null
   ...
   */
  private parsePropertySignature(node: ts.PropertySignature, sourceFile: ts.SourceFile): PropertySignature | null {
    const name = this.getNodeName(node)
    if (!name) return null
   ...
   */
  private parseTypeDeclaration(node: ts.TypeAliasDeclaration, sourceFile: ts.SourceFile): TypeInfo {
    const name = node.name.text
    const line = sourceFile.getLineAndCharacterOfPosition(node.getStart()).line + 1
  ...
  private getNodeName(node: ts.Node): string | undefined {
    ...
    if ('name' in nodeWithName && nodeWithName.name && ts.isIdentifier(nodeWithName.name)) {
  ...
  private hasExportModifier(node: ts.Node): boolean {
    if ('modifiers' in node && Array.isArray((node as any).modifiers)) {
      return (node as any).modifiers.some((modifier: ts.Modifier) => 
