/**
 * Dependency graph builder for ResonanceAI MVP
 *
...
export class DependencyGraphBuilder {
		...
		for (const [filePath, fileInfo] of this.fileMap) {
			for (const importInfo of fileInfo.imports) {
				try {
					...
					console.warn(
						`Failed to resolve import "${importInfo.module}" in ${filePath}`,
					);
				}
			}
		}
				...
				if (await FileUtils.exists(packageJsonPath)) {
					try {
						const packageJson = JSON.parse(
							await FileUtils.readFile(packageJsonPath),
						);
	...
	private calculateMetrics(): void {
		...
		const averageDependencies = totalNodes > 0 ? edges.length / totalNodes : 0;
